...
class PresaleElementPage(BasePage):
    ...
    def verify_separate_sale_status_was_aborted_approval(self):
    ...
    def go_to_approval_presale(self):
        assert self.is_element_clickable(
            *PresaleElementLocators.SEND_APPROVAL_BUTTON), \
            'Кнопка "Отправить на согласование не доступна" не доступна для нажатия'
        self.browser.find_element(*PresaleElementLocators.SEND_APPROVAL_BUTTON).click()
        self.browser.switch_to.frame(self.browser.find_element(*PresaleElementLocators.iframe))
        self.browser.find_element(*PresaleElementLocators.APPROVAL_DIRECTION_ELEMENT).click()
